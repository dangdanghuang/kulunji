...
void Read_Sensors(){
  ...
  CSIL = 0.0000;      //Clear Previous Output Current 
  ...
  for(int i = 0; i<avgCountCS; i++){
    CSIL = CSIL + ads.computeVolts(ads.readADC_SingleEnded(0)); ////负载电流检测
    CSI = CSI + ads.computeVolts(ads.readADC_SingleEnded(2));
  }
  CSI_converted = (CSI/avgCountCS)*1.3300;
  CSIL_converted = (CSIL/avgCountCS)*1.3300;/////负载电流检测
  currentInput  = ((CSI_converted-currentMidPoint)*-1)/currentSensV;
  loadcurrentOutput = ((CSIL_converted-currentMidPoint)*-1)/currentSensV;/////负载电流检测
  ...
  batteryPercent  = constrain(batteryPercent,0,100);//限制电池容量百分比为0-100
